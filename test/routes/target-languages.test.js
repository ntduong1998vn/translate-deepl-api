...
test('GET /target-languages returns list of target languages', async (t) => {
  ...
  if (payload.languages.length > 0) {
    ...
    assert.ok(lang.name)
    assert.ok(lang.code)
    assert.ok(typeof lang.supportsFormality === 'boolean')
  }
})

test('GET /target-languages handles API errors gracefully', async (t) => {
  const app = await build(t)
